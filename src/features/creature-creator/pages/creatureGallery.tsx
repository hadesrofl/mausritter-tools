...
export default function CreatureGallery() {
  ...
  const [creatureLocalStorage, setCreatureLocalStorage] = useLocalStorageState(
    "mausritter-creatures",
    "",
  );
  const [creatures, setCreatures] = useState<Creature[]>(
    creatureLocalStorage ? JSON.parse(creatureLocalStorage) : [],
  );
  const dialogs = useDialogs();
  const { t: translateCreatureCreator } = useTranslation(
    TranslationsCreatureCreator,
  );
  const { t: translateRules } = useTranslation(TranslationsRuleTerms);
  ...
  const handleUpdateClick = async (creature: Creature) => {
    ...
    if (newCreature) {
      const indexOfCreature = newCreatures.findIndex(
        (c) => c.id === creature.id,
      );
  ...
  const creatureGallery = () => {
    return creatures.map((creature) => {
      return (
        <Grid size={6} key={creature.name}>
          <Statblock
            creature={creature}
            theme={theme}
            onUpdate={handleUpdateClick}
            onDelete={handleCreatureDelete}
            t={translateRules}
          />
        </Grid>
      );
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={5} textAlign="end" alignContent="center">
          <Button
            ...
            startIcon={<Save />}
          >
            {translateCreatureCreator("buttons.saveInBrowser")}
          </Button>
        </Grid>
